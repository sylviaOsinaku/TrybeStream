...
function TrendMoviePage() {
  const [page, setPage] = React.useState(1);
  const [movieTrendData, setMovieTrendData] = useState([]);
  ...
  const getTrendingMovies = useCallback(
    async function () {
      try {
        ...
        setMovieTrendData(theMovies?.results);
      } catch (err) {
        console.log(err);
      }
    },
  ...
  return (
    <Stack spacing={"2"}>
      <MoviePagination movies={movieTrendData} pageHeading="Trending Movies" />
